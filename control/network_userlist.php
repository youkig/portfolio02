...
$sql = "select * from t_network";
$n = $dbh->prepare($sql);
$n -> execute();
$result = $n -> fetchAll(PDO::FETCH_ASSOC);
if ($result){
	$sql = "select max(indate) as max from t_network";
	$n = $dbh->prepare($sql);
	$n -> execute();
	$rsmax = $n -> fetch(PDO::FETCH_ASSOC);
	$year1 = date('Y',strtotime($rsmax["max"]));

	$sql = "select min(indate) as min from t_network";
	$n = $dbh->prepare($sql);
	$n -> execute();
...
<div class="block">
	<form action="network_userlist.php">
		...
		<p><input type="submit" value="検索" />　<a href="network_userlist.php">全件表示</a></p>
...
$sql = "select t_user.name,t_user.company,t_user.taikai,t_network.*,t_state.state as statename ";
$sqlf = " From t_user inner join t_network on t_user.id=t_network.uid left join t_state on t_network.state=t_state.id";

$sqlw = " Where t_network.id<>0"; 
...
if($nowpoint-$webmax>=0){
	echo '<a href="network_userlist.php?point='.$nowpoint-$webmax.'">前のページへ　</a>';
}
if($nowpoint+$webmax<$count){
	echo '<a href="network_userlist.php?point='.$nowpoint+$webmax.'">次のページへ</a>';
}
?></p>
	<table>
	<thead>
	...
	<tbody>
		...
		<tr<?=$classname?>>
			...
			<td class="centering"><a href="network_userdisp.php?id=<?=$id?>">編集</a><input type="hidden" name="userids" value="<?=$id?>" /></td>
		
			<td class="centering"><?= (!empty($rs["company"])) ? "法人" : "個人"?></td>

			<td><a href="network_userdisp.php?id=<?=$id?>"><?php
			if (!empty($rs["name"])){
			echo $rs["name"];
			}
			
			if (!empty($rs["company"])){echo "<br />" . $rs["company"];}
