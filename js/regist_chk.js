...
//お問合せ内容
	if(formname.koumoku1){
		...
		errms=errms+missch2(in_name,"お問合せ項目");
	}
...
//性別
	if(formname.sex){
		...
		errms=errms+missch2(in_name,"性別");
	}	
...
//法人名登録管理者
	if(formname.hname){
		...
		if(in_name.length!=0){
			in_name3 = formname.fcompany.value;	
			
			errms=errms+missch(in_name3,"法人名ふりがな");
				
			in_name2 = formname.hname.value;	
			
			errms=errms+missch(in_name2,"登録責任者");
...
//住所のチェック
	if(formname.address){
		...
		errms=errms+missch(in_name,"ご住所");
	}	
...
//パスワードのチェック
	if(formname.password){
		...
		}else if(in_name.length<8 || in_name.length>12){
		errms=errms+"パスワードは8文字から12文字で入力してください\n";
		}
...
//出荷予定品目のチェック
	if(formname.comment){
		in_name = formname.comment.value;
		errms=errms+missch(in_name,"出荷予定品目");
	}
...
//出荷予定曜日のチェック	
	if(formname.youbi1){
		...
		for(var i=1;i<10;i++){
			...
			if(document.getElementById('youbi'+[i]).checked){
