...
//お問合せ内容
	if(formname.koumoku1){
		var in_name="";
		if(formname.koumoku1.checked){
			in_name = formname.koumoku1.value;
		}
		if(formname.koumoku2.checked){
			in_name = formname.koumoku2.value;
		}
		if(formname.koumoku3.checked){
			in_name = formname.koumoku3.value;
		}
        if(formname.koumoku4.checked){
			in_name = formname.koumoku4.value;
		}
        if(formname.koumoku5.checked){
			in_name = formname.koumoku5.value;
		}
        if(formname.koumoku6.checked){
			in_name = formname.koumoku6.value;
		}
        if(formname.koumoku7.checked){
			in_name = formname.koumoku7.value;
		}
        if(formname.koumoku8.checked){
			in_name = formname.koumoku8.value;
		}
        if(formname.koumoku9.checked){
			in_name = formname.koumoku9.value;
		}
		errms=errms+missch2(in_name,"ご利用予定サービス");
	}
...
//パスワードのチェック
	if(formname.password){
		in_name = formname.password.value;
		errms=errms+missch(in_name,"パスワード");
	}

	if(formname.password){
		in_pass = formname.password.value;
	if(in_pass.length <= 7 || in_pass.length >= 17){
		errms=errms+"パスワードは8～16文字以内で入力してください。\n";
	}
	}
    
    
//電話番号チェック
	if(formname.tel){//タグが存在するかチェック。存在しないならば判定しない。
		in_wpd_phone = formname.tel.value;
		errms=errms+phonech(in_wpd_phone,"","電話");
	}    
...
//住所のチェック
	if(formname.address){
		...
		errms=errms+missch(in_name,"市区町村名");
	}
