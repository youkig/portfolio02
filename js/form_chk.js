...
//お問合せ内容
	if(formname.koumoku1){
          ...
          var in_name2=0;
        var in_name = document.getElementsByName("koumoku[]");
    
        for(let i=0;i< in_name.length;i++){
            if(in_name[i].checked){
            in_name2++;
            }
        }
        //console.log(in_name2);
        if(in_name2==0){
        errms=errms+"お問合せ項目を選択してください。\n";
        }
...
//電話番号のチェック
	if(formname.tel){
		in_name = formname.tel.value;
		errms=errms+missch(in_name,"電話番号");
	}
...
//問い合わせのチェック
	if(formname.comment){
		...
		errms=errms+missch(in_name,"お問合わせ内容");
	}
