...
//予約日
    if(formname.ydate){
        var in_name="";
        in_name = formname.ydate.value;
        if(in_name.length==0){
		errms=errms+"ご予約日を選択してください。\n";
        }
    }
...
//開始時間
    if(formname.hour){
        var in_name="";
        var in_name2="";
        in_name = formname.hour.value;
        in_name2 = formname.time.value;
        if(in_name.length==0 || in_name2.length==0){
        errms=errms+"開始時間を選択してください。\n";
        }
        if(in_name=="8" && (in_name2=="0" || in_name2=="10" || in_name2=="20")){
        errms=errms+"8時台は30分以降を選択ください。\n";
        }
    }
...
//カゴ
    if(formname.kago){
        var in_name="";
        in_name = formname.kago.value;
        if(in_name.length==0){
		errms=errms+"ご利用のカゴ数を選択してください。\n";
        }
    } 
...
//お弁当
    if(formname.obento){
        var in_name="";
        var in_name2="";
        if(formname.obento.checked){
            in_name=formname.obento.value;
        }
        if(in_name.length!=0){
            in_name2 = formname.obentokazu.value;
            if(in_name2.length==0){
            errms=errms+"お弁当のご予約個数を入力してください。\n";
            }
        }
    }
