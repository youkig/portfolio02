...
//住所のチェック
	if(formname.address2){
		...
		errms=errms+missch(in_name,"以下住所");
	}	
...
//メールチェック
    ...
    if(formname.kemail){
        in_name = formname.email.value;
        in_name2 = formname.kemail.value;
        
        if(in_name != in_name2){
            errms=errms+"確認メールアドレスが違います\n";
        }
    }
    ...
    if(formname.adult){
        in_name = formname.adult.value;
        in_name2 = formname.child.value;
        if(in_name.lenth==0 || in_name2.length==0){
        errms=errms+missch(in_name,"大人または子供の人数");
        }
    }
